# flag
Parsing command-line flags in C
...
int main(int argc, char **argv) {
  bool *help = flag_bool("help", false, "Print help to stdout and exit.");
  char **addr = flag_str("addr", "localhost:8080", "The server address");
  long int *port = flag_int("port", 8080, "The port to listen on");
  float *gravity = flag_float("gravity", 9.8f, "Gravity");
  bool *use_tls = flag_bool("tls", false, "Use TLS");
  size_t *storage = flag_size("storage", 500, "The amount of storage");
...
```
See [Example](flag_test.c "Example usage for the flag package.")

Inspired by [Tsoding](https://github.com/tsoding/flag.h/blob/master/flag.h)'s flag.h package and YT live stream.
